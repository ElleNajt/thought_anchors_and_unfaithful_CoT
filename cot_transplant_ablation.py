#!/usr/bin/env python3
"""
CoT Transplant Ablation Experiment

This script builds on cot_transplant.py to ablate individual model components
and measure their impact on steering towards the hinted answer.

For each component (attention head or MLP layer):
1. Run transplant experiment with component ablated
2. Measure steering towards hinted answer
3. Compare to baseline (no ablation)
4. Rank components by ablation-induced drop in steering

Output: CSV with ranked components and plot of top 10
"""

import os
import re
import sys
import pandas as pd
import numpy as np
from transformers import AutoModelForCausalLM, AutoTokenizer
import torch
import matplotlib.pyplot as plt
from tqdm import tqdm
from typing import Dict, List, Tuple, Optional
import einops
import argparse

# Add the demo folder to path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'CoT_Faithfulness_demo'))
from load_CoT import load_all_problems

# Import from original file
from cot_transplant import count_answer_letters, extract_cot_sentences


class ComponentAblator:
    """
    Handles ablation of model components (attention heads and MLP layers).
    """
    
    def __init__(self, model):
        self.model = model
        self.hooks = []
        self.ablation_cache = {}
        
    def clear_hooks(self):
        """Remove all hooks."""
        for hook in self.hooks:
            hook.remove()
        self.hooks = []
        
    def ablate_attention_head(self, layer_idx: int, head_idx: int):
        """
        Ablate a specific attention head by zeroing its output.
        """
        def hook_fn(module, input, output):
            # output is typically (batch, seq, n_heads, head_dim)
            # We need to zero out the specific head
            if isinstance(output, tuple):
                attn_output = output[0]
            else:
                attn_output = output
                
            # Clone to avoid in-place modification issues
            attn_output = attn_output.clone()
            
            # Zero out the specific head
            # Shape depends on model architecture
            if len(attn_output.shape) == 4:  # (batch, seq, n_heads, head_dim)
                attn_output[:, :, head_idx, :] = 0
            elif len(attn_output.shape) == 3:  # (batch, seq, hidden_dim)
                # Need to figure out head dimension
                hidden_dim = attn_output.shape[-1]
                n_heads = self._get_num_heads(layer_idx)
                head_dim = hidden_dim // n_heads
                start_idx = head_idx * head_dim
                end_idx = start_idx + head_dim
                attn_output[:, :, start_idx:end_idx] = 0
                
            if isinstance(output, tuple):
                return (attn_output,) + output[1:]
            else:
                return attn_output
                
        # Get the attention module
        layer = self._get_layer(layer_idx)
        attn_module = self._get_attention_module(layer)
        
        hook = attn_module.register_forward_hook(hook_fn)
        self.hooks.append(hook)
        
    def ablate_mlp(self, layer_idx: int):
        """
        Ablate an entire MLP layer by zeroing its output.
        """
        def hook_fn(module, input, output):
            # Zero out MLP output
            if isinstance(output, tuple):
                return (torch.zeros_like(output[0]),) + output[1:]
            else:
                return torch.zeros_like(output)
                
        layer = self._get_layer(layer_idx)
        mlp_module = self._get_mlp_module(layer)
        
        hook = mlp_module.register_forward_hook(hook_fn)
        self.hooks.append(hook)
        
    def _get_layer(self, layer_idx: int):
        """Get the transformer layer at the given index."""
        # Try common model architectures
        if hasattr(self.model, 'model'):
            base_model = self.model.model
        else:
            base_model = self.model
            
        if hasattr(base_model, 'layers'):
            return base_model.layers[layer_idx]
        elif hasattr(base_model, 'h'):
            return base_model.h[layer_idx]
        elif hasattr(base_model, 'transformer'):
            if hasattr(base_model.transformer, 'h'):
                return base_model.transformer.h[layer_idx]
            elif hasattr(base_model.transformer, 'layers'):
                return base_model.transformer.layers[layer_idx]
        else:
            raise ValueError("Could not find transformer layers in model")
            
    def _get_attention_module(self, layer):
        """Get the attention module from a layer."""
        if hasattr(layer, 'self_attn'):
            return layer.self_attn
        elif hasattr(layer, 'attn'):
            return layer.attn
        elif hasattr(layer, 'attention'):
            return layer.attention
        else:
            raise ValueError("Could not find attention module in layer")
            
    def _get_mlp_module(self, layer):
        """Get the MLP module from a layer."""
        if hasattr(layer, 'mlp'):
            return layer.mlp
        elif hasattr(layer, 'feed_forward'):
            return layer.feed_forward
        elif hasattr(layer, 'ffn'):
            return layer.ffn
        else:
            raise ValueError("Could not find MLP module in layer")
            
    def _get_num_heads(self, layer_idx: int):
        """Get the number of attention heads."""
        layer = self._get_layer(layer_idx)
        attn = self._get_attention_module(layer)
        
        if hasattr(attn, 'num_heads'):
            return attn.num_heads
        elif hasattr(attn, 'n_head'):
            return attn.n_head
        elif hasattr(attn, 'num_attention_heads'):
            return attn.num_attention_heads
        else:
            # Try to infer from config
            config = self.model.config
            if hasattr(config, 'num_attention_heads'):
                return config.num_attention_heads
            elif hasattr(config, 'n_head'):
                return config.n_head
            else:
                raise ValueError("Could not determine number of attention heads")
                
    def get_num_layers(self):
        """Get the total number of transformer layers."""
        if hasattr(self.model, 'model'):
            base_model = self.model.model
        else:
            base_model = self.model
            
        if hasattr(base_model, 'layers'):
            return len(base_model.layers)
        elif hasattr(base_model, 'h'):
            return len(base_model.h)
        elif hasattr(base_model, 'transformer'):
            if hasattr(base_model.transformer, 'h'):
                return len(base_model.transformer.h)
            elif hasattr(base_model.transformer, 'layers'):
                return len(base_model.transformer.layers)
        else:
            # Try config
            config = self.model.config
            if hasattr(config, 'num_hidden_layers'):
                return config.num_hidden_layers
            elif hasattr(config, 'n_layer'):
                return config.n_layer
            else:
                raise ValueError("Could not determine number of layers")


def run_transplant_with_ablation(
    problem_data: Dict,
    model,
    tokenizer,
    ablator: ComponentAblator,
    component_type: str,
    component_idx: Tuple[int, ...],
    start_sentence: int = 3,
    num_sentences: int = 5,
    n_samples: int = 5,
    max_tokens: int = 1024,
    device: str = "cuda"
) -> Tuple[List[int], List[str]]:
    """
    Run transplant experiment with a specific component ablated.
    
    Args:
        problem_data: Problem dictionary
        model: Language model
        tokenizer: Tokenizer
        ablator: ComponentAblator instance
        component_type: 'attn_head' or 'mlp'
        component_idx: (layer_idx, head_idx) for attn_head, (layer_idx,) for mlp
        start_sentence: Starting sentence for transplant
        num_sentences: Number of sentences to transplant
        n_samples: Number of samples to generate
        max_tokens: Max tokens to generate
        device: Device to use
        
    Returns:
        Tuple of (answer_counts, answer_letters)
    """
    # Clear previous hooks
    ablator.clear_hooks()
    
    # Apply ablation
    if component_type == 'attn_head':
        layer_idx, head_idx = component_idx
        ablator.ablate_attention_head(layer_idx, head_idx)
    elif component_type == 'mlp':
        layer_idx = component_idx[0]
        ablator.ablate_mlp(layer_idx)
    else:
        raise ValueError(f"Unknown component type: {component_type}")
    
    # Extract transplant section
    transplant_section = extract_cot_sentences(
        problem_data["reasoning_text"],
        start_sentence=start_sentence,
        num_sentences=num_sentences
    )
    
    # Build transplanted prompt
    transplanted_prompt = problem_data["question"] + transplant_section + " "
    
    # Tokenize
    tokens = tokenizer.encode(transplanted_prompt, return_tensors="pt").to(device)
    many_tokens = einops.repeat(tokens, "1 seq -> n seq", n=n_samples)
    
    # Generate with ablation active
    with torch.no_grad():
        outputs = model.generate(
            many_tokens,
            max_new_tokens=max_tokens,
            do_sample=True,
            temperature=0.7,
            pad_token_id=tokenizer.eos_token_id
        )
    
    # Decode and count answers
    generated_texts = tokenizer.batch_decode(outputs, skip_special_tokens=False)
    answer_counts, answer_letters = count_answer_letters(generated_texts, tokenizer)
    
    # Clear hooks after generation
    ablator.clear_hooks()
    
    return answer_counts, answer_letters


def compute_steering_score(answer_counts: List[int], cue_answer: str, n_samples: int) -> float:
    """
    Compute steering score: proportion of answers matching the hinted (cue) answer.
    
    Args:
        answer_counts: [count_A, count_B, count_C, count_D, count_other]
        cue_answer: The hinted answer letter ('A', 'B', 'C', or 'D')
        n_samples: Total number of samples
        
    Returns:
        Steering score (0 to 1)
    """
    cue_idx = ord(cue_answer) - ord('A')
    return answer_counts[cue_idx] / n_samples


def run_ablation_experiment(
    problem_data: Dict,
    model,
    tokenizer,
    n_samples: int = 5,
    max_tokens: int = 1024,
    device: str = "cuda",
    layer_start: int = 0,
    layer_end: Optional[int] = None,
    sample_rate: float = 1.0,
    test_mlps: bool = True
) -> pd.DataFrame:
    """
    Run ablation experiment on all components.
    
    Args:
        problem_data: Problem dictionary
        model: Language model
        tokenizer: Tokenizer
        n_samples: Number of samples per experiment
        max_tokens: Max tokens to generate
        device: Device to use
        layer_start: First layer to test (inclusive)
        layer_end: Last layer to test (exclusive), None = all layers
        sample_rate: Fraction of attention heads to test (0-1), 1.0 = all heads
        test_mlps: Whether to test MLP layers
        
    Returns:
        DataFrame with results for each component
    """
    ablator = ComponentAblator(model)
    
    # First, get baseline (no ablation)
    print("Running baseline (no ablation)...")
    baseline_counts, baseline_letters = run_transplant_with_ablation(
        problem_data, model, tokenizer, ablator,
        component_type='mlp',  # Dummy values, no hooks will be applied
        component_idx=(0,),
        n_samples=n_samples,
        max_tokens=max_tokens,
        device=device
    )
    baseline_steering = compute_steering_score(
        baseline_counts, problem_data['cue_answer'], n_samples
    )
    print(f"Baseline steering towards {problem_data['cue_answer']}: {baseline_steering:.3f}")
    print(f"Baseline answers: {baseline_letters}")
    
    # Get model dimensions
    num_layers = ablator.get_num_layers()
    num_heads = ablator._get_num_heads(0)  # Assume same for all layers
    
    if layer_end is None:
        layer_end = num_layers
    
    # Calculate what we'll test
    layers_to_test = list(range(layer_start, layer_end))
    heads_per_layer = max(1, int(num_heads * sample_rate))
    
    if sample_rate < 1.0:
        print(f"\n⚡ SAMPLING MODE: Testing {heads_per_layer}/{num_heads} heads per layer")
    
    total_attn = len(layers_to_test) * heads_per_layer
    total_mlp = len(layers_to_test) if test_mlps else 0
    total_components = total_attn + total_mlp
    
    print(f"\nModel has {num_layers} layers, {num_heads} heads per layer")
    print(f"Testing layers {layer_start} to {layer_end-1}")
    print(f"Total components to test: {total_attn} attention heads + {total_mlp} MLPs = {total_components}")
    
    # Estimate runtime
    # Rough estimate: 2-3 seconds per sample generation
    time_per_component = n_samples * 2.5  # seconds
    total_time_seconds = total_components * time_per_component
    total_time_hours = total_time_seconds / 3600
    print(f"Estimated runtime: {total_time_hours:.1f} hours ({total_time_seconds/60:.0f} minutes)")
    
    results = []
    
    # Test each attention head
    print("\nTesting attention heads...")
    for layer_idx in layers_to_test:
        # Sample heads if needed
        if sample_rate < 1.0:
            heads_to_test = np.random.choice(num_heads, heads_per_layer, replace=False)
            heads_to_test = sorted(heads_to_test)
        else:
            heads_to_test = range(num_heads)
        
        for head_idx in tqdm(heads_to_test, desc=f"Layer {layer_idx}/{layer_end-1}", leave=True):
            try:
                answer_counts, answer_letters = run_transplant_with_ablation(
                    problem_data, model, tokenizer, ablator,
                    component_type='attn_head',
                    component_idx=(layer_idx, head_idx),
                    n_samples=n_samples,
                    max_tokens=max_tokens,
                    device=device
                )
                
                steering = compute_steering_score(
                    answer_counts, problem_data['cue_answer'], n_samples
                )
                
                # Negative change means ablation reduced steering (component was important)
                steering_change = steering - baseline_steering
                
                results.append({
                    'component_type': 'attn_head',
                    'layer': layer_idx,
                    'head': head_idx,
                    'component_name': f'L{layer_idx}H{head_idx}',
                    'baseline_steering': baseline_steering,
                    'ablated_steering': steering,
                    'steering_change': steering_change,
                    'steering_drop': -steering_change,  # Positive = important component
                    'answer_letters': ''.join(answer_letters),
                })
            except Exception as e:
                print(f"\nError testing L{layer_idx}H{head_idx}: {e}")
                continue
    
    # Test each MLP layer
    if test_mlps:
        print("\nTesting MLP layers...")
        for layer_idx in tqdm(layers_to_test, desc="MLP layers"):
            try:
                answer_counts, answer_letters = run_transplant_with_ablation(
                    problem_data, model, tokenizer, ablator,
                    component_type='mlp',
                    component_idx=(layer_idx,),
                    n_samples=n_samples,
                    max_tokens=max_tokens,
                    device=device
                )
                
                steering = compute_steering_score(
                    answer_counts, problem_data['cue_answer'], n_samples
                )
                
                steering_change = steering - baseline_steering
                
                results.append({
                    'component_type': 'mlp',
                    'layer': layer_idx,
                    'head': -1,  # N/A for MLPs
                    'component_name': f'L{layer_idx}_MLP',
                    'baseline_steering': baseline_steering,
                    'ablated_steering': steering,
                    'steering_change': steering_change,
                    'steering_drop': -steering_change,
                    'answer_letters': ''.join(answer_letters),
                })
            except Exception as e:
                print(f"\nError testing MLP layer {layer_idx}: {e}")
                continue
    
    # Create DataFrame and sort by steering drop (descending)
    df = pd.DataFrame(results)
    df = df.sort_values('steering_drop', ascending=False).reset_index(drop=True)
    
    return df


def plot_top_components(df: pd.DataFrame, top_n: int = 10, output_path: str = 'cot_transplant_ablation.png'):
    """
    Plot the top N components by steering drop.
    
    Args:
        df: Results DataFrame
        top_n: Number of top components to plot
        output_path: Path to save the plot
    """
    top_df = df.head(top_n)
    
    fig, ax = plt.subplots(figsize=(12, 6))
    
    colors = ['#e74c3c' if ct == 'attn_head' else '#3498db' 
              for ct in top_df['component_type']]
    
    bars = ax.bar(range(len(top_df)), top_df['steering_drop'], color=colors)
    
    ax.set_xlabel('Component', fontsize=12)
    ax.set_ylabel('Steering Drop (Baseline - Ablated)', fontsize=12)
    ax.set_title(f'Top {top_n} Components by Impact on CoT Transplant Steering', fontsize=14, fontweight='bold')
    ax.set_xticks(range(len(top_df)))
    ax.set_xticklabels(top_df['component_name'], rotation=45, ha='right')
    ax.grid(axis='y', alpha=0.3, linestyle='--')
    
    # Add legend
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor='#e74c3c', label='Attention Head'),
        Patch(facecolor='#3498db', label='MLP Layer')
    ]
    ax.legend(handles=legend_elements, loc='upper right')
    
    # Add value labels on bars
    for i, (bar, value) in enumerate(zip(bars, top_df['steering_drop'])):
        height = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2., height,
                f'{value:.3f}',
                ha='center', va='bottom', fontsize=9)
    
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\nPlot saved to: {output_path}")
    plt.close()


def main():
    """
    Main function to run CoT transplant ablation experiment.
    """
    parser = argparse.ArgumentParser(description='CoT Transplant Ablation Experiment')
    parser.add_argument('--n-samples', type=int, default=5,
                        help='Number of samples per component (default: 5)')
    parser.add_argument('--max-tokens', type=int, default=1024,
                        help='Max tokens to generate (default: 1024)')
    parser.add_argument('--layer-start', type=int, default=0,
                        help='First layer to test (default: 0)')
    parser.add_argument('--layer-end', type=int, default=None,
                        help='Last layer to test, exclusive (default: all layers)')
    parser.add_argument('--sample-rate', type=float, default=1.0,
                        help='Fraction of attention heads to sample per layer (default: 1.0 = all)')
    parser.add_argument('--no-mlps', action='store_true',
                        help='Skip testing MLP layers')
    parser.add_argument('--problem-idx', type=int, default=0,
                        help='Problem index to use (default: 0)')
    parser.add_argument('--output-prefix', type=str, default='cot_transplant_ablation',
                        help='Prefix for output files (default: cot_transplant_ablation)')
    
    args = parser.parse_args()
    
    print("="*80)
    print("CoT TRANSPLANT ABLATION EXPERIMENT")
    print("="*80)
    
    # Load model
    model_name = "deepseek-ai/DeepSeek-R1-Distill-Qwen-14B"
    print(f"\nLoading {model_name}...")
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = AutoModelForCausalLM.from_pretrained(
        model_name,
        torch_dtype=torch.float16,
        device_map="auto"
    )
    model.eval()
    
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Using device: {device}")
    
    # Load problems
    print("\nLoading problem data...")
    original_dir = os.getcwd()
    demo_dir = os.path.join(os.path.dirname(__file__), 'CoT_Faithfulness_demo')
    os.chdir(demo_dir)
    try:
        data = load_all_problems()
        print(f"Loaded {len(data)} problems")
    finally:
        os.chdir(original_dir)
    
    # Choose a problem
    problem = data[args.problem_idx]
    
    print(f"\nSelected problem #{problem['pn']}")
    print(f"Ground truth answer: {problem['gt_answer']}")
    print(f"Hinted answer: {problem['cue_answer']}")
    
    # Run ablation experiment
    print("\n" + "="*80)
    print("RUNNING ABLATION EXPERIMENT")
    print("="*80)
    
    results_df = run_ablation_experiment(
        problem, model, tokenizer, 
        n_samples=args.n_samples,
        max_tokens=args.max_tokens,
        device=device,
        layer_start=args.layer_start,
        layer_end=args.layer_end,
        sample_rate=args.sample_rate,
        test_mlps=not args.no_mlps
    )
    
    # Save results
    output_csv = f'{args.output_prefix}_results.csv'
    results_df.to_csv(output_csv, index=False)
    print(f"\n✓ Results saved to: {output_csv}")
    
    # Display top components
    print("\n" + "="*80)
    print("TOP 20 COMPONENTS BY STEERING DROP")
    print("="*80)
    print(results_df.head(20).to_string(index=False))
    
    # Plot top 10
    plot_output = f'{args.output_prefix}_plot.png'
    plot_top_components(results_df, top_n=10, output_path=plot_output)
    
    # Summary statistics
    print("\n" + "="*80)
    print("SUMMARY STATISTICS")
    print("="*80)
    print(f"Baseline steering towards hinted answer: {results_df['baseline_steering'].iloc[0]:.3f}")
    print(f"\nTop component: {results_df.iloc[0]['component_name']}")
    print(f"  Steering drop: {results_df.iloc[0]['steering_drop']:.3f}")
    print(f"  Ablated steering: {results_df.iloc[0]['ablated_steering']:.3f}")
    
    attn_results = results_df[results_df['component_type']=='attn_head']
    mlp_results = results_df[results_df['component_type']=='mlp']
    
    if len(attn_results) > 0:
        print(f"\nAttention heads:")
        print(f"  Mean steering drop: {attn_results['steering_drop'].mean():.3f}")
        print(f"  Std steering drop: {attn_results['steering_drop'].std():.3f}")
        print(f"  Max steering drop: {attn_results['steering_drop'].max():.3f}")
    
    if len(mlp_results) > 0:
        print(f"\nMLP layers:")
        print(f"  Mean steering drop: {mlp_results['steering_drop'].mean():.3f}")
        print(f"  Std steering drop: {mlp_results['steering_drop'].std():.3f}")
        print(f"  Max steering drop: {mlp_results['steering_drop'].max():.3f}")
    
    print("\n" + "="*80)
    print("EXPERIMENT COMPLETE")
    print("="*80)


if __name__ == "__main__":
    main()

